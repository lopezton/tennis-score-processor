...
public class AceStatisticInstructionTest extends AbstractProcessingTests {
	...
	public void t1_defaultScenario() {
		...
		matchProcessor.addStatisticInstruction(new AceStatisticInstruction(player1));
	...
	public void t2_commonStatisticProcessor() {
		...
		for (int i = 1; i <=4; i++) {
			this.hitFirstServeAce(match, player1);
			assertThat(statProcessor.getNumberOfAces(player1).getCount(), is(i));
		}
		
		for (int i = 1; i <=4; i++) {
			this.hitFirstServeAce(match, player2);
			assertThat(statProcessor.getNumberOfAces(player1).getCount(), is(4));
		}
		
		this.hitFirstServeAce(match, player1);
		assertThat(statProcessor.getNumberOfAces(player1).getCount(), is(5));
	}
}
